...
type SidebarData = Locales["common"]["sidebar"];

const Link = ({ ...props }: DetailedHTMLProps<AnchorHTMLAttributes<HTMLAnchorElement>, HTMLAnchorElement>) =>
  <a className="text-blue-700 hover:underline print:text-inherit" {...props} />;

const SidebarSectionRenderer = ({ title, children }: any) => (
  <div className="flex flex-col gap-1">
    <header><h3 className="font-medium text-lg">{title}</h3></header>
    <div>
      {children}
    </div>
  </div>
  ...
  <SidebarSectionRenderer title={title}>
    <div className="flex flex-col">
      {map(data, ({ href, icon, title, shortUrl }, i) => {
        ...
        return (
          <a className="flex gap-2 text-blue-700 hover:underline print:text-inherit" href={href} key={i}>
...
const SidebarSection = ({ sectionKey, props }: {
...
export const Sidebar = () => {
  ...
  return (
    <div className="flex flex-col print:flex-row print:[&>*]:flex-1 print:gap-6 print:text-sm">
